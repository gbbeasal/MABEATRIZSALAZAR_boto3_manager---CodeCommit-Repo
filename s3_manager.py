...
def create_bucket_object(bucket_name, file_path, key_prefix=None):
    ...
    bucket = get_bucket('testawesomeproject5-bea')
...
def main():
    ...
    create_bucket('testawesomeproject5-bea', region=None)
    print("\n\n 2. LISTING BUCKETS: ")
    list_buckets()
...
if __name__ == '__main__':
    main()
    print(f'\n\n 3. GETTING BUCKETS : ')
    cdate = get_bucket('testawesomeproject5-bea').creation_date
    print('Bucket Creation Date :', cdate)
    tmp_file = create_tempfile()
    b_obj = create_bucket_object('testawesomeproject5-bea', tmp_file, key_prefix='temp/')
    ...
    print("\n\n 6. UPLOADING FILE TO THE SAME BUCKET OBJECT :")
    create_bucket_object('testawesomeproject5-bea', tmp_file.name, key_prefix='temp/')
    p1 = list(get_bucket('testawesomeproject5-bea').objects.all())
    print("All bucket object latest versions: ", p1)
    p2 = list(get_bucket('testawesomeproject5-bea').object_versions.all())
    print("\nAll bucket object versions: ", p2)
    
    print("\n\n 7. FILTERING BUCKET OBJECT :")
    # First let's create other objects in the bucket
    for _ in range(3):
        obj = create_bucket_object(
        'testawesomeproject5-bea',
        file_path=create_tempfile(),
        key_prefix='others/'
        )
        print(f'Object {obj.key} created!')
    p3 = list(get_bucket('testawesomeproject5-bea').objects.all())
    print("\nAll bucket object latest version: ", p3)
    p4 = list(get_bucket('testawesomeproject5-bea').objects.filter(Prefix='temp/'))
    print("\nAll bucket object latest version - Filtered: ", p4)
    
    print("\n\n 8. DELETING BUCKET OBJECT :")
    get_bucket('testawesomeproject5-bea').objects.filter(Prefix='temp/').delete()
    list(get_bucket('testawesomeproject5-bea').object_versions.all())
    d1 = delete_bucket_objects('testawesomeproject5-bea', key_prefix='temp/')
    print("\nwith key_prefix: ", d1)
    d2 = delete_bucket_objects('testawesomeproject5-bea')
    print("\nwithout key_prefix: ", d2)
    
    print("\n\n 9. DELETING BUCKET :")
    print('Buckets BEFORE deletion: ')
    list_buckets()
    delete_buckets('testawesomeproject5-bea')
